...
void *create_files(void *id_points){
    int id = *(int *)id_points;
    int files_to_create = num_files / num_threads;
    int start_file = id * files_to_create;
    ...
    for (int i = start_file; i < end_file; i++) {
        ...
        FILE *f = fopen(file, "w");
        printf("Hilo \033[01;33m%i\033[0m ha creando fichero \033[01;34m%s\033[0m\n", id, file);
        fclose(f);
    }
